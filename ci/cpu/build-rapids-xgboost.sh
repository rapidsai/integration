...
########################################################
# rapids-xgboost meta pkg conda build script for gpuCI #
########################################################
set -ex
...
function build_pkg {
  ...
  conda build -c ${CONDA_USERNAME:-rapidsai-nightly} -c nvidia -c conda-forge --python=${PYTHON_VERSION} conda/recipes/rapids-xgboost

  # Get output location
  gpuci_logger "Get conda build output..."
  export UPLOADFILE=`conda build -c ${CONDA_USERNAME:-rapidsai-nightly} -c nvidia -c conda-forge --python=${PYTHON_VERSION} conda/recipes/rapids-xgboost --output`
  test -e ${UPLOADFILE}
