...
conda build -c rapidsai-nightly -c nvidia -c conda-forge --python=${PYTHON_VERSION} conda/recipes/rapids-xgboost

# Get output location
gpuci_logger "Get conda build output..."
export UPLOADFILE=`conda build -c rapidsai-nightly -c nvidia -c conda-forge --python=${PYTHON_VERSION} conda/recipes/rapids-xgboost --output`
test -e ${UPLOADFILE}
