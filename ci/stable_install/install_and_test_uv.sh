...
source "${SCRIPT_DIR}/bootstrap/uv.sh"
...
function createPyEnv {
    ...
    rapids-logger "Creating uv virtualenv for Python $PY_VER"
    uv init --python="$PY_VER" --name="stable_rapids_py${PY_VER}_cu${CUDA_VERSION}" "$INSTALL_DIR"
    pushd $INSTALL_DIR
}
...
rapids-logger "Adding upstream PyPI wheels to uv lock file for Python $PYTHON_VERSION and CUDA $CUDA_VERSION"
uv add "${UV_INSTALL_PYPI[@]}"

rapids-logger "Adding NVIDIA PyPI only wheels to uv lock file for Python $PYTHON_VERSION and CUDA $CUDA_VERSION"
uv add "${UV_INSTALL_NVIDIA_PYPI[@]}" --index https://pypi.nvidia.com
...
uv add git+https://github.com/rapidsai/rapids-cli
